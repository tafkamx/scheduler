  ...
  Object.keys(resources).forEach(function (resourceName) {
    ...
    Object.keys(resources[resourceName]).forEach(function (roleName) {
      ...
      fixedRole.actions.forEach(function (actionName) {
        Sc.ACL.setRule(actionName, resourceName, roleName, function (acl, req) {
          return fixedRole[actionName](acl, req);
        }, 'TYPE_ALLOW');
      });

      // finally, we can create our middleware per-resource and actions
      fixedMiddleware[resourceName] = fixedRole.actions.map(function (actionName) {
        return Sc.ACL.can(actionName, resourceName);
      });
    });
  });
