...
var Neonode = Class({}, 'Neonode')({
  prototype : {
    ...
    init : function (){
      ...
      logger.info("Execute neonode._serverStart() to start the server");
    ...
    loadControllers : function(){
      ...
      logger.info('Loading BaseMailer');
      require(path.join(__dirname, 'mailers', 'BaseMailer.js'));
      ...
      glob.sync("controllers/**/*.js").forEach(function(file) {
        ...
        var controllerName = controller.name ||
          // take the controllerName from the filename if missing
          fileNameArray[fileNameArray.length - 1].replace(/\.js|Controller/g, '').replace();
