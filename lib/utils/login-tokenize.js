...
const REASON = 'cross-install-or-branch-login';
const DURATION = 1000 * 60 * 2;
const TOKEN_DELIMETER = '|';
const DEFAULT_BRANCH_NAME = 'default'; // TODO: Remove when default branch only has installation in subdomain.
...
LoginTokenize.createInstallationToken = function(user, installationName, timestamp) {
  ...
  var nonce = nonces.createNonce(REASON, DURATION, grantee, installationName);
  // TODO: Remove `DEFAULT_BRANCH_NAME` part when default branch only has installation in subdomain.
  var uniqueSignature = `${secret}${nonce}${installationName}-${DEFAULT_BRANCH_NAME}${timestamp}`; 
...
LoginTokenize.createBranchToken = function(user, installationName, branchName, timestamp) {
  ...
  var nonce = nonces.createNonce(REASON, DURATION, grantee, installationName, branchName);
...
LoginTokenize.validateToken = function(b64EncodedToken, installationName, branchName) {
  ...
  branchName = branchName ? branchName : 'default'; // TODO: Remove when default branch only has installation in subdomain.
  var now = Date.now();
  ...
  var data = nonces.validateNonce(nonce, REASON);
  if (!data) return false;
  var uniqueSignature = `${secret}${nonce}${installationName}${branchName && branchName ? '-' + branchName : ''}${timestamp}`;
