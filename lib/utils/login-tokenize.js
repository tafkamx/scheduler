...
(function($) { // Below, $ is the same thing as `module.exports`.
  ...
  $.generateInstallToken = function(installToLogin, time) {
    ...
    var nonce = nonces.get_user_nonce('cross-install-or-branch-login'); // Get nonce based on current User IP or session key (used for referencing )
  ...
  $.attemptVerifyToken = function(token, currentInstallOrBranch) {
    ...
    if (time - Date.now() > 1000 * 60) return false; // Make sure time set is not older than 1 minute
    if (!(userData = nonces.verify_nonce(nonce, 'cross-install-or-branch-login'))) return false; // Make sure nonce value is good
    ...
    // TODO
    // userData contains what was in `req.ser` in the referring connection
    // Set separate cookie/session variable that is related to an object of data for the User to log in as, unless it times out after 1 hour of inactivity.
  };

  $.doLogin = function(userData) {
    // Set event to run on request (possibly not needed if `$.attemptVerify` can be called within `SessionController.js`)
  };
})(module.exports);
