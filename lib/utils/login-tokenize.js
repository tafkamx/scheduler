...
(function($) { // Below, $ is the same thing as `module.exports`.
   ...
   *
   * @param object req            (required) The current request object. Can use helper `req` in templates
   ...
   */
  $.generateInstallToken = function(req, installToLogin, time) {
    // If not set, time = current time
    time = time || Date.now(); // If not set, make time current timestamp
    var nonce = nonces.getUserNonce(req, 'cross-install-or-branch-login'); // Get nonce based on current User IP or session key (used for referencing )
    var salt = bcrypt.genSaltSync(4);
    var hash = bcrypt.hashSync(new Buffer(secret + nonce + installToLogin + time).toString('utf8'), salt); // Generate hash with userID, secret key, nonce, and time
  ...
  $.attemptVerifyToken = function(token, currentInstallOrBranch) {
    ...
    if (time - Date.now() > 1000 * 60 * 60) {
      ...
      return false; // Make sure time set is not older than 1 minute
    }
    if (!(userData = nonces.verifyNonce(nonce, 'cross-install-or-branch-login'))) {
      //console.log('Bad nonce')
      return false; // Make sure nonce value is good
    }
